 ...
 */
export default function ResourceReadiness({ readinessData, isLoading = false }) {
  const { t } = useLanguage();
  ...
  if (!readinessData) {
    return (
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center text-slate-500 text-xs">
        No resource inventory data available for this farm.
      </div>
    );
  }

  const { farmName, overallStatus } = readinessData;

  return (
    <div className="space-y-6">
      {/* 1. Overall Readiness Status Banner */}
      <section
